...
export class LivApproveService {
  ...
  updateApprovalTask(livrequestId: any, status: any, rejectReason: string): Observable<any> {
    const url = `${environment.apiUrl}/Customer/updateApprovalTask`;
    const params = { livrequestId, Status: status,  RejectReason: rejectReason }; // Assuming Status is case-sensitive
